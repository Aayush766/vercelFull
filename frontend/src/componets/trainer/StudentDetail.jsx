...
const StudentDetail = () => {
  ...
  const generateRandomQuizData = () => {
    const totalQuestions = 10;
    const score = Math.floor(Math.random() * (totalQuestions + 1)); // Random score between 0 and totalQuestions
    ...
    const questions = Array.from({ length: totalQuestions }, (_, i) => ({
      ...
      answer: ['Option A', 'Option B', 'Option C', 'Option D'][Math.floor(Math.random() * 4)], // Random correct answer
    }));

    const answers = {};
    questions.forEach(q => {
      // Simulate user answers: 70% chance of being correct if score allows, otherwise incorrect
      if (Math.random() < (score / totalQuestions) && score > 0) {
        answers[q.id] = q.answer;
      } else {
        const wrongOptions = q.options.filter(opt => opt !== q.answer);
        answers[q.id] = wrongOptions[Math.floor(Math.random() * wrongOptions.length)]; // Random wrong answer
      }
    });

    const startedAt = new Date(Date.now() - Math.random() * 3600000); // Up to 1 hour ago
  ...
  // Use location.state if available, otherwise generate random data
  const { score, total, questions, answers, startedAt, completedAt } =
    location.state || generateRandomQuizData();
